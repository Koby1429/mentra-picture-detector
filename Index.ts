...
const WEBVIEW_HTML = [
  ...
  'body{font-family:Arial,sans-serif;text-align:center;padding:16px;background:#1a1a2e;color:#eee;min-height:100vh}',
  ...
  '.btn{width:100%;max-width:340px;padding:16px;margin:6px auto;display:block;border:none;border-radius:10px;cursor:pointer;font-size:17px;font-weight:bold}',
  ...
  '#previewSection{display:none;max-width:360px;margin:12px auto;text-align:left}',
  '#previewImg{width:100%;border-radius:10px;border:3px solid #4CAF50;display:block}',
  ...
  '#progressBar{width:100%;max-width:340px;margin:6px auto;background:#333;border-radius:6px;overflow:hidden;height:10px;display:none}',
  ...
  'function setFb(m){el("feedback").textContent=m}',
  'function setFb2(m){el("feedback2").textContent=m}',
  'function showProgress(on){el("progressBar").style.display=on?"block":"none";if(!on)el("progressFill").style.width="0%"}',
  'function setProgress(p){el("progressFill").style.width=p+"%"}',
  ...
  '  var label=pct>=70?"Good quality":pct>=40?"Acceptable - may work":"Low quality - consider retaking";',
  ...
  '    showProgress(false);',
  '    if(d.error){setFb("Error: "+d.error);}',
  '    else{showPreview(d.base64,d.sizeKB,d.quality);setFb("Photo ready - confirm or retake");}',
  '  }catch(e){showProgress(false);setFb("Capture failed - try again");}',
  '  btn.disabled=false;',
  ...
  '  }catch(e){',
  '    clearInterval(ticker);showProgress(false);',
  ...
  '  var c=el("results");c.innerHTML="";',
  '  results.forEach(function(r,i){',
  '    var thumb=r.base64?"<img class=\\"thumb\\" src=\\"data:image/jpeg;base64,"+r.base64+"\\" alt=\\"match\\"/>":"";',
  '    var score=Math.round(r.score);',
  '    c.innerHTML+="<div class=\\"result-card\\">"+thumb',
  ...
  '      +"<div class=\\"row\\"><span class=\\"lbl\\">URL</span><span class=\\"val\\"><a href=\\""+r.url+"\\" target=\\"_blank\\" rel=\\"noopener\\">View page</a></span></div>"',
  '      +"</div>";',
  '  });',
  '}',
...
class FaceAnalyzerApp extends AppServer {
  constructor(options: any) {
    ...
    app.get('/webview', (_req, res) => {
      ...
      res.status(200).send(WEBVIEW_HTML);
    });
